...
class Star_Eyed_Trainer(BotAI):
    ...
    def __init__(self, actor, actor_optim, tracker, device: str='cuda'):    
        self.actor = actor
    ...
    async def on_step(self, iteration:int):
        if iteration % 100 == 0:
            ...
            '''for tumor in self.structures(UnitTypeId.CREEPTUMORBURROWED)[:5]:
                if await self.can_cast(tumor, AbilityId.BUILD_CREEPTUMOR_TUMOR, only_check_energy_and_cooldown=True):
                    tumor_vector = self.enemy_start_locations[0] - tumor.position
                    tumor_vector = 10*(tumor_vector/((tumor_vector[0]**2 + tumor_vector[1]**2)**0.5))
                    rotation = random.uniform(-math.pi/4, math.pi/4)
                    loc = tumor.position + Point2((math.cos(rotation) * tumor_vector[0] + math.sin(rotation) * tumor_vector[1], -math.sin(rotation) * tumor_vector[0] + math.cos(rotation) * tumor_vector[1]))
                    tumor(AbilityId.BUILD_CREEPTUMOR_TUMOR, target=loc, queue=True)
            '''
        ...
        value = self.actor.forward_critic(self.current_state, self.current_map)
        action_dist = self.actor(self.current_state, self.current_map)
        ...
        self.tracker.increment(self.curr_reward)
        ...
        if iteration % N == 0 and iteration != 0:
            self.frac *= .99985
            lrnow = self.frac * learning_rate_actor
            ...
            for _ in range(n_epochs):
                ...
                for batch in batches:                 
                    ...
                    # Actor Loss
                    dist = self.actor(observations[batch], maps[batch])
                    critic_value = self.actor.forward_critic(observations[batch], maps[batch]).squeeze()
